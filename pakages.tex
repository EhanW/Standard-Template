% encoding
\usepackage[utf8]{inputenc}

% float environment
\usepackage{float}

% for figures, wrap figures, and wrap tables
\usepackage{caption}
\usepackage{subcaption}
\usepackage{wrapfig}
\usepackage{graphicx}

% custom table
\usepackage{multirow}
\usepackage{booktabs}
\usepackage{colortbl}
\usepackage{longtable} % cross-page tables
\usepackage{array}

% math
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{mathtools}
\usepackage{amsthm}
\usepackage{nicefrac} % slanted fractions
\usepackage{derivative} % derivative, partial derivative, total derivative
\usepackage{empheq} % custom equations


% custom font, number, and icon
\usepackage{bm} % bold font
\usepackage{mathrsfs} % scr font
\usepackage{dsfont} % ds font
\usepackage{pifont}
\usepackage{fontawesome5} % special icons
\usepackage{latexsym} % special icons
\usepackage{marvosym}  % special icons

% filler text 
\usepackage{lipsum}

% colors 
\usepackage[dvipsnames]{xcolor}

% comments
\usepackage{comment}

% python code
\usepackage{pythonhighlight}
\usepackage{listings}

% author affiliation
\usepackage{authblk}

% hyperlink reference and clever reference
\usepackage[colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue]{hyperref}
\usepackage{cleveref}
% \usepackage[capitalize,noabbrev]{cleveref}

% enumerate and itermize
\usepackage{enumitem}

% add URL
\usepackage{url}

% fullpage
\usepackage{fullpage}

% algorithm
\usepackage{algorithm}
\usepackage{algorithmicx}
\usepackage[noend]{algpseudocode}
\makeatletter
\renewcommand{\ALG@beginalgorithmic}{\footnotesize} % foot size in algorithm
\makeatother


% use biblatex
% \usepackage[backend=biber,style=authoryear,sorting=nyt,giveninits=true,uniquename=false,uniquelist=false,maxcitenames=2,maxbibnames=9,defernumbers=true,url=false,doi=false,isbn=false]{biblatex}
% \usepackage[utf8]{inputenc}
% \usepackage{csquotes}
% \usepackage[style=authoryear,backend=biber]{biblatex}
% \addbibresource{reference.bib}
% \printbibliography



% import commands
\input{commands}
